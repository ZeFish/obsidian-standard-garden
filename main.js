...
`).forEach((o,r)=>{let s=e.createEl("p",{text:o,cls:r===0?"stnd-modal-message":"stnd-modal-detail"})});let t=e.createEl("div",{cls:"stnd-modal-btns"});t.createEl("button",{text:"Cancel",cls:"stnd-modal-btn-cancel"}).addEventListener("click",()=>{this.close(),this.onCancel()}),t.createEl("button",{text:this.confirmText,cls:"mod-cta"}).addEventListener("click",()=>{this.close(),this.onConfirm()})}onClose(){this.contentEl.empty()}};at.exports={StndConfirmModal:he}});var rt=O((ea,ot)=>{"use strict";var ae=require("obsidian"),me=class extends ae.Modal{constructor(e,t){super(e),this.plugin=t}onOpen(){let{contentEl:e}=this;e.addClass("stnd-modal"),e.addClass("stnd-ask-modal"),e.createEl("h2",{text:"\u2728 Ask Hyphe",cls:"stnd-modal-title"}),e.createEl("p",{text:"Ask Hyphe a question about your notes. Hyphe will search across your digital garden to answer.",cls:"stnd-modal-detail"});let t=e.createEl("textarea",{cls:"stnd-modal-textarea",placeholder:"e.g., What did I learn about permaculture recently?"});t.style.width="100%",t.style.height="100px",t.style.marginBottom="15px",t.style.padding="10px",t.style.borderRadius="4px",t.style.border="1px solid var(--background-modifier-border)",t.style.background="var(--background-primary)",t.style.color="var(--text-normal)";let a=e.createEl("div",{cls:"stnd-modal-result-container"});a.style.display="none",a.style.marginTop="15px",a.style.padding="15px",a.style.borderRadius="4px",a.style.background="var(--background-secondary)",a.style.borderLeft="4px solid var(--interactive-accent)",a.style.maxHeight="300px",a.style.overflowY="auto";let n=a.createEl("div",{cls:"stnd-modal-result-text"});n.style.lineHeight="1.5",n.style.color="var(--text-normal)";let o=e.createEl("div",{cls:"stnd-modal-btns"});o.createEl("button",{text:"Close",cls:"stnd-modal-btn-cancel"}).addEventListener("click",()=>{this.close()});let s=o.createEl("button",{text:"Ask Hyphe",cls:"mod-cta"}),i=e.createEl("div",{cls:"stnd-ask-horizon"});i.setAttribute("aria-hidden","true"),i.innerHTML=`
    ...
    `,s.addEventListener("click",async()=>{let l=t.value.trim();if(!l){new ae.Notice("Please enter a question.");return}s.disabled=!0,t.disabled=!0,s.text="Hyphe is searching...",a.style.display="block",n.empty(),n.setText("Hyphe is searching your notes and generating an answer..."),n.style.fontStyle="italic";try{let d=this.plugin.settings.apiKey,c=await ae.requestUrl({url:`${this.plugin.settings.apiUrl}/ai/ask`,method:"POST",headers:{"Content-Type":"application/json","x-api-key":d},body:JSON.stringify({question:l}),throw:!1});if(c.status<200||c.status>=300){let f=c.text;throw new Error(f||`Server error (${c.status})`)}let h=c.json;n.style.fontStyle="normal",h.answer?(n.empty(),await ae.MarkdownRenderer.renderMarkdown(h.answer,n,"",this),n.querySelectorAll("a").forEach(f=>{f.addEventListener("click",u=>{let m=f.getAttribute("href");if(m&&m.startsWith("/")){u.preventDefault();let g=m.replace(/^\/+|\/+$/g,""),y=this.plugin.garden?.bySlug?.get(g)||this.plugin.garden?.byTitleSlug?.get(g)||this.plugin.garden?.byBasenameSlug?.get(g);y?(this.app.workspace.getLeaf(!1).openFile(y),this.close()):this.plugin.settings.apiUsername&&window.open(`https://standard.garden${m}`,"_blank")}})})):n.setText("No response was returned by Hyphe.")}catch(d){n.style.fontStyle="normal",n.setText(`Error: ${d.message}`),console.error("Error during Hyphe query:",d)}finally{s.disabled=!1,t.disabled=!1,s.text="Ask Hyphe"}})}onClose(){this.contentEl.empty()}};ot.exports={StndAskModal:me}});var X=O((ta,it)=>{"use strict";var H=require("obsidian"),{isPublishIntent:st}=q(),oe={unpublished:{icon:"cloud-off",color:"var(--text-faint)",label:"Non publi\xE9"},pending:{icon:"upload-cloud",color:"var(--color-orange)",label:"\xC0 publier (pas encore en ligne)"},public:{icon:"globe",color:"var(--color-green)",label:"Public"},unlisted:{icon:"eye-off",color:"var(--color-yellow)",label:"Non list\xE9"},private:{icon:"lock",color:"var(--color-blue)",label:"Priv\xE9"},outdated:{icon:"arrow-down-circle",color:"var(--color-orange)",label:"Mise \xE0 jour disponible en ligne"},changed:{icon:"upload-cloud",color:"var(--color-blue)",label:"Modifications locales non publi\xE9es"}},ge=class{constructor(e,t){this.app=e,this.plugin=t,this.statusBarEl=null,this.ribbonEl=null,this.noteStatuses=new Map}async load(){let e=()=>this.refreshAll();this.plugin.registerEvent(this.app.workspace.on("active-leaf-change",e)),this.plugin.registerEvent(this.app.workspace.on("layout-change",e)),this.plugin.registerEvent(this.app.metadataCache.on("changed",t=>this.refreshForFile(t))),this.app.workspace.onLayoutReady(e)}async unload(){this.cleanupAll()}cleanupAll(){this.app.workspace.getLeavesOfType("markdown").forEach(e=>{let t=e.view&&e.view._stndPublishAction;t&&(t.remove(),delete e.view._stndPublishAction);let a=e.view&&e.view._stndBottomIndicator;a&&(a.remove(),delete e.view._stndBottomIndicator)}),this.statusBarEl&&(this.statusBarEl.remove(),this.statusBarEl=null),this.ribbonEl&&(this.ribbonEl.remove(),this.ribbonEl=null)}stateKey(e,t){let a=e||{};if(!st(a))return"unpublished";if(!a["garden-url"]&&!a.url_public)return"pending";if(t&&this.noteStatuses){let r=this.noteStatuses.get(t);if(r&&(r.status==="outdated"||r.status==="changed"))return r.status}let o=String(a.visibility||"").toLowerCase().trim();return o==="private"?"private":o==="unlisted"?"unlisted":"public"}async triggerStatusCheck(e){if(!e||!!!this.plugin.settings.apiKey)return;let a=this.app.metadataCache.getFileCache(e)?.frontmatter||{},n=st(a),o=a["garden-url"]!=null||a.url_public!=null;if(!n||!o){this.noteStatuses.delete(e.path);return}let r=this.noteStatuses.get(e.path),s=Date.now();if(r&&s-r.timestamp<1e4&&e.stat.mtime<=r.timestamp)return;this.noteStatuses.set(e.path,{status:r?r.status:this.stateKey(a,e.path),remoteContent:r?r.remoteContent:null,timestamp:s});let i=this.plugin.garden;if(i)try{let l=await i.checkNoteStatus(e);this.noteStatuses.set(e.path,{status:l.status,remoteContent:l.remoteContent,timestamp:Date.now()}),this.refreshAll()}catch(l){console.error("Standard : Erreur lors de la v\xE9rification asynchrone du statut :",l)}}refreshAll(){let e=!!this.plugin.settings.apiKey,t=this.plugin.settings.publishStatusLocation||"titlebar",a=this.plugin.settings.publishIndicatorStyle||"garden";if(!e&&a==="hidden"){this.cleanupAll();return}let n=this.app.workspace.getActiveFile();n&&e&&this.triggerStatusCheck(n),(t!=="titlebar"||!e||t==="hidden")&&this.app.workspace.getLeavesOfType("markdown").forEach(o=>{let r=o.view&&o.view._stndPublishAction;r&&(r.remove(),delete o.view._stndPublishAction)}),(t!=="statusbar"||!e||t==="hidden")&&this.statusBarEl&&(this.statusBarEl.remove(),this.statusBarEl=null),(t!=="ribbon"||!e||t==="hidden")&&this.ribbonEl&&(this.ribbonEl.remove(),this.ribbonEl=null),e&&t!=="hidden"&&(t==="titlebar"?this.app.workspace.getLeavesOfType("markdown").forEach(o=>this.refreshLeaf(o)):t==="statusbar"?this.refreshStatusBar():t==="ribbon"&&this.refreshRibbon()),this.app.workspace.getLeavesOfType("markdown").forEach(o=>{this.refreshBottomIndicator(o,a)})}refreshForFile(e){let t=this.app.workspace.getActiveFile();if(t&&t.path===e.path)this.refreshAll();else{let a=this.plugin.settings.publishIndicatorStyle||"garden";this.app.workspace.getLeavesOfType("markdown").forEach(n=>{n.view&&n.view.file&&n.view.file.path===e.path&&(this.plugin.settings.publishStatusLocation==="titlebar"&&this.refreshLeaf(n),this.refreshBottomIndicator(n,a))})}}refreshBottomIndicator(e,t,a=!1){let n=e?.view;if(!n||!n.containerEl||!n.file)return;let o=n._stndBottomIndicator;if(t==="hidden"){o&&(o.remove(),delete n._stndBottomIndicator);return}let r=this.app.metadataCache.getFileCache(n.file)?.frontmatter||null,s=this.stateKey(r,n.file.path);(!o||!o.isConnected)&&(o=document.createElement("div"),o.className="stnd-bottom-indicator",n.containerEl.appendChild(o),n._stndBottomIndicator=o),o.className=`stnd-bottom-indicator stnd-style-${t} stnd-state-${s}`;let i=oe[s]?.label||s;o.setAttribute("title",`Garden: ${i}`),a&&t==="garden"&&(o.classList.remove("stnd-growing"),o.offsetWidth,o.classList.add("stnd-growing"))}refreshLeaf(e){let t=e.view;if(!t||typeof t.addAction!="function"||!t.file)return;let a=this.app.metadataCache.getFileCache(t.file)?.frontmatter||null,n=this.stateKey(a,t.file.path),o=oe[n],r=t._stndPublishAction;(!r||!r.isConnected)&&(r=t.addAction(o.icon,"Garden Status",s=>this.onClick(t,s)),r.addClass("stnd-publish-status"),t._stndPublishAction=r),H.setIcon(r,o.icon),r.style.color=o.color,r.setAttribute("aria-label",`Garden Status \u2014 ${o.label}`),r.dataset.stndState=n}refreshStatusBar(){let e=this.app.workspace.getActiveFile();if(!e){this.statusBarEl&&(this.statusBarEl.style.display="none");return}let t=this.app.metadataCache.getFileCache(e)?.frontmatter||null,a=this.stateKey(t,e.path),n=oe[a];this.statusBarEl||(this.statusBarEl=this.plugin.addStatusBarItem(),this.statusBarEl.addClass("stnd-publish-status-statusbar"),this.statusBarEl.style.cursor="pointer",this.statusBarEl.addEventListener("click",r=>{let s=this.app.workspace.getActiveViewOfType(H.MarkdownView);s&&this.onClick(s,r)})),this.statusBarEl.style.display="",this.statusBarEl.empty();let o=this.statusBarEl.createSpan();H.setIcon(o,n.icon),o.style.color=n.color,o.style.marginRight="6px",o.style.display="inline-flex",o.style.alignItems="center",this.statusBarEl.createSpan({text:`Garden: ${n.label}`}),this.statusBarEl.setAttribute("aria-label",`Garden Status \u2014 ${n.label}`)}refreshRibbon(){let e=this.app.workspace.getActiveFile();if(!e){this.ribbonEl&&(this.ribbonEl.style.display="none");return}let t=this.app.metadataCache.getFileCache(e)?.frontmatter||null,a=this.stateKey(t,e.path),n=oe[a];this.ribbonEl||(this.ribbonEl=this.plugin.addRibbonIcon(n.icon,"Garden Status",o=>{let r=this.app.workspace.getActiveViewOfType(H.MarkdownView);r&&this.onClick(r,o)}),this.ribbonEl.addClass("stnd-publish-status-ribbon")),this.ribbonEl.style.display="",H.setIcon(this.ribbonEl,n.icon),this.ribbonEl.style.color=n.color,this.ribbonEl.setAttribute("aria-label",`Garden Status \u2014 ${n.label}`)}onClick(e,t){let a=e.file;if(!a)return;let n=this.plugin.garden;if(!n)return;let o=this.app.metadataCache.getFileCache(a)?.frontmatter||null,r=this.stateKey(o,a.path),s=new H.Menu,i=async()=>{if(!n.checkApiKeyAndShowModal())return;let l=await n.publishWithCheck(a);if(l!==null){if(new H.Notice(l?`Standard : "${a.basename}" publi\xE9.`:`Standard : \xC9chec de la publication de "${a.basename}".`),l){let d=e.leaf||{view:e},c=this.plugin.settings.publishIndicatorStyle||"garden";this.refreshBottomIndicator(d,c,!0)}l&&n.plugin.settings.openAfterPublish&&n.viewLiveVersion(a)}this.refreshAll()};if(r==="unpublished"||r==="pending")s.addItem(l=>l.setTitle("Publier dans le jardin").setIcon("upload-cloud").onClick(()=>i()));else if(r==="outdated"){let d=this.noteStatuses.get(a.path)?.remoteContent||"";s.addItem(c=>c.setTitle("T\xE9l\xE9charger la mise \xE0 jour (\xE9craser le fichier local)").setIcon("arrow-down-circle").onClick(async()=>{d?(await this.app.vault.modify(a,d),new H.Notice("Standard : Fichier local mis \xE0 jour avec la version en ligne."),this.noteStatuses.set(a.path,{status:"synced",timestamp:Date.now()}),this.refreshAll()):new H.Notice("Standard : Contenu distant introuvable.")})),s.addItem(c=>c.setTitle("Forcer la publication locale").setIcon("refresh-cw").onClick(()=>i())),s.addItem(c=>c.setTitle("Voir en ligne").setIcon("external-link").onClick(()=>n.viewLiveVersion(a))),s.addSeparator(),s.addItem(c=>c.setTitle("Retirer du jardin (d\xE9-publier)").setIcon("trash-2").setWarning(!0).onClick(async()=>{await n.deleteOnlineVersion(a),this.refreshAll()}))}else r==="changed"?(s.addItem(l=>l.setTitle("Publier les modifications locales").setIcon("upload-cloud").onClick(()=>i())),s.addItem(l=>l.setTitle("Voir en ligne").setIcon("external-link").onClick(()=>n.viewLiveVersion(a))),s.addSeparator(),s.addItem(l=>l.setTitle("Retirer du jardin (d\xE9-publier)").setIcon("trash-2").setWarning(!0).onClick(async()=>{await n.deleteOnlineVersion(a),this.refreshAll()}))):(s.addItem(l=>l.setTitle("Voir en ligne").setIcon("external-link").onClick(()=>n.viewLiveVersion(a))),s.addItem(l=>l.setTitle("Re-publier").setIcon("refresh-cw").onClick(()=>i())),s.addSeparator(),s.addItem(l=>l.setTitle("Retirer du jardin (d\xE9-publier)").setIcon("trash-2").setWarning(!0).onClick(async()=>{await n.deleteOnlineVersion(a),this.refreshAll()})));s.addSeparator(),s.addItem(l=>l.setTitle("Synchroniser toutes les notes").setIcon("folder-sync").onClick(async()=>{await n.syncAllPublished();let d=this.app.workspace.getActiveViewOfType(H.MarkdownView);if(d){let c=d.leaf||{view:d},h=this.plugin.settings.publishIndicatorStyle||"garden";this.refreshBottomIndicator(c,h,!0)}})),s.showAtMouseEvent(t)}};it.exports={PublishStatusFeature:ge}});var ct=O((na,dt)=>{"use strict";var R=require("obsidian"),{KNOWN_TOKENS:$t,isPublishIntent:G,isImageFile:re,isPdfFile:lt,isAttachmentFile:ye,getMimeType:en}=q(),{StndConfirmModal:we}=ue(),{StndAskModal:tn}=rt();function I(p){return String(p).normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[æ]/gi,"ae").replace(/[œ]/gi,"oe").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")}var Z=class{constructor(e,t){this.app=e,this.files=t,this.byNanoId=new Map,this.bySlug=new Map,this.byTitleSlug=new Map,this.byBasenameSlug=new Map,this.byGardenUrl=new Map;for(let a of t){let n=e.metadataCache.getFileCache(a)?.frontmatter||{};if(n["garden-short"]){let s=String(n["garden-short"]).match(/stnd\.gd\/([a-zA-Z0-9_-]+)/);s&&this.byNanoId.set(s[1],a)}let o=n.permalink??n.slug;if(o!=null){let s=String(o).replace(/^\/+|\/+$/g,""),i=s===""?"~root":s;this.bySlug.set(i,a)}if(n.title){let s=I(n.title);s&&this.byTitleSlug.set(s,a)}let r=I(a.basename);if(r&&this.byBasenameSlug.set(r,a),n["garden-url"]){let s=String(n["garden-url"]).toLowerCase().trim().replace(/\/$/,"");this.byGardenUrl.set(s,a)}}}findMatchForRemote(e){if(!e)return null;if(e.nano_id&&this.byNanoId.has(e.nano_id))return this.byNanoId.get(e.nano_id);if(e.slug&&this.bySlug.has(e.slug))return this.bySlug.get(e.slug);if(e.slug&&this.byTitleSlug.has(e.slug))return this.byTitleSlug.get(e.slug);if(e.slug&&this.byBasenameSlug.has(e.slug))return this.byBasenameSlug.get(e.slug);if(e.title){let t=I(e.title);if(t){if(this.bySlug.has(t))return this.bySlug.get(t);if(this.byTitleSlug.has(t))return this.byTitleSlug.get(t);if(this.byBasenameSlug.has(t))return this.byBasenameSlug.get(t)}}return null}},be=class extends R.Modal{constructor(e,t,a){super(e),this.total=t,this.breakdown=a||{},this.cancelled=!1,this.finished=!1,this.syncedNotes=[],this.pulledNotes=[],this.createdNotes=[],this.unpublishedNotes=[],this.skippedNotes=[],this.failedNotes=[],this.logs=[],this.startTime=Date.now()}onOpen(){let{contentEl:e,titleEl:t}=this;e.addClass("stnd-modal"),t.setText("Standard \u2014 Synchronisation du jardin"),this.statusEl=e.createEl("p",{text:`Pr\xE9paration... 0 / ${this.total}`,cls:"stnd-modal-message"});let a=e.createDiv();a.style.cssText="height:8px;border-radius:4px;background:var(--background-modifier-border);overflow:hidden;margin:0.5em 0;",this.barEl=a.createDiv(),this.barEl.style.cssText="height:100%;width:0%;background:var(--interactive-accent);transition:width .15s ease;";let n=this.breakdown,o=e.createDiv();o.style.cssText="display:flex;flex-wrap:wrap;gap:6px;margin:0.5em 0 0.75em;";let r=(l,d,c)=>{if(!d)return;let h=o.createEl("span");h.style.cssText=`display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:12px;font-size:var(--font-ui-smaller);background:var(--background-secondary);color:${c};border:1px solid currentColor;opacity:.85;`,h.setText(`${l} ${d}`)};r("\u2191 \xC0 publier",n.toPublish,"var(--color-green)"),r("\u2212 \xC0 d\xE9publier",n.toUnpublish,"var(--color-orange)"),r("\u2193 \xC0 t\xE9l\xE9charger",n.toCreate,"var(--color-blue)"),this.currentEl=e.createEl("div",{cls:"stnd-modal-detail",text:"V\xE9rification des notes existantes..."}),this.currentEl.style.cssText="opacity:.7;font-size:var(--font-ui-smaller);min-height:1.4em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;";let s=e.createEl("div",{text:"Journal de synchronisation :",cls:"stnd-modal-detail"});s.style.cssText="margin-top: 0.75em; margin-bottom: 0.25em; font-weight: 600; font-size: var(--font-ui-smaller);",this.logContainer=e.createDiv(),this.logContainer.style.cssText="height: 160px; overflow-y: auto; background: var(--background-primary-alt); border: 1px solid var(--background-modifier-border); border-radius: 6px; padding: 6px 10px; font-family: var(--font-monospace); font-size: 11px; line-height: 1.5;",this.reconciliationEl=e.createEl("div"),this.reconciliationEl.style.cssText="margin-top: 0.75em; max-height: 180px; overflow-y: auto; font-size: var(--font-ui-smaller);";let i=e.createEl("div",{cls:"stnd-modal-btns"});this.copyBtn=i.createEl("button",{text:"Copier le log",cls:"stnd-modal-btn-cancel"}),this.copyBtn.addEventListener("click",()=>{if(this.logs.length===0){new R.Notice("Le journal est vide pour le moment.");return}let l=this.logs.map(d=>`[${d.time}] [${d.tag}] ${d.name}${d.detail?" \u2014 "+d.detail:""}`).join(`
`);navigator.clipboard.writeText(l),new R.Notice(`Journal copi\xE9 (${this.logs.length} entr\xE9es) !`)}),this.actionBtn=i.createEl("button",{text:"Annuler",cls:"mod-warning"}),this.actionBtn.addEventListener("click",()=>{if(this.finished)return this.close();this.cancelled=!0,this.actionBtn.disabled=!0,this.actionBtn.setText("Annulation...")})}update({index:e,current:t,synced:a,pulled:n,created:o,unpublished:r,skipped:s,failed:i}){if(!this.barEl)return;let l=this.total?Math.round(e/this.total*100):0;this.barEl.style.width=l+"%";let d=a+n+o+r;this.statusEl.setText(`${e} / ${this.total} (${l}%) \u2014 ${d} action(s), ${s} identique(s)`+(i?`, ${i} \xE9chou\xE9(s)`:"")),this.currentEl.setText(t?`Traitement : ${t}`:"")}recordResult(e,t,a=""){let n=new Date().toLocaleTimeString(),o="INFO",r="var(--text-muted)",s="var(--text-normal)";if(e==="synced"?(this.syncedNotes.push(t),o="\u2191 PUBLI\xC9",r="var(--color-green)"):e==="pulled"?(this.pulledNotes.push(t),o="\u2193 T\xC9L\xC9CHARG\xC9",r="var(--color-blue)"):e==="created"?(this.createdNotes.push(t),o="+ CR\xC9\xC9",r="var(--color-blue)"):e==="unpublished"?(this.unpublishedNotes.push(t),o="\u2212 D\xC9PUBLI\xC9",r="var(--color-orange)"):e==="skipped"?(this.skippedNotes.push(t),o="\u25CB IDENTIQUE",r="var(--text-faint)",s="var(--text-muted)"):e==="failed"&&(this.failedNotes.push({name:t,detail:a}),o="\u2717 \xC9CHEC",r="var(--color-red)",s="var(--color-red)"),this.logs.push({time:n,tag:o,type:e,name:t,detail:a}),this.logContainer){let i=this.logContainer.createDiv();i.style.cssText="display: flex; gap: 6px; align-items: baseline; word-break: break-all; margin-bottom: 2px;";let l=i.createSpan();l.style.cssText="color: var(--text-faint); font-size: 10px; flex-shrink: 0;",l.setText(n);let d=i.createSpan();d.style.cssText=`color: ${r}; font-weight: 600; flex-shrink: 0; font-size: 10px;`,d.setText(o);let c=i.createSpan();c.style.cssText=`color: ${s}; flex: 1;`,c.setText(t+(a?` \u2014 ${a}`:"")),this.logContainer.scrollTop=this.logContainer.scrollHeight}}done({synced:e,pulled:t,created:a,unpublished:n,skipped:o,failed:r,notesOnline:s,stats:i}){if(this.finished=!0,!this.barEl)return;let l=Math.round((Date.now()-this.startTime)/1e3);this.barEl.style.width="100%";let d=this.cancelled?"Annul\xE9e":"Termin\xE9e";this.statusEl.setText(`Synchronisation ${d.toLowerCase()} \xB7 ${l}s`),this.currentEl.setText(""),this.reconciliationEl.empty();let c=this.reconciliationEl.createEl("h4",{text:"Rapport de r\xE9conciliation :",cls:"stnd-reconciliation-title"});c.style.cssText="margin: 0 0 0.5em 0;";let h=s??e+o,f=i?.imagesReused||0,u=i?.imagesUploaded||0,m=i?.imagesFailed||0,g=f+u,y=r||0,v=y+m,k=i&&i.imagesChecked>0?` + ${g} image(s) en ligne${f>0?` (${f} d\xE9dupliqu\xE9e(s))`:""}`:"",x=this.reconciliationEl.createDiv({cls:"stnd-reconciliation-summary"});if(x.style.cssText="padding: 10px 14px; border-radius: 8px; background: var(--background-secondary-alt); border: 1px solid var(--background-modifier-border); margin-bottom: 12px; font-weight: 500; font-size: var(--font-ui-small);",v===0)x.setText(`\u2713 ${h} note(s)${k}, 0 manquante.`);else{let L=[];y>0&&L.push(`${y} note(s) manquante(s)`),m>0&&L.push(`${m} image(s) non t\xE9l\xE9vers\xE9e(s)`),x.setText(`\u26A0 ${h} note(s)${k}, ${L.join(", ")}.`)}let C=this.reconciliationEl.createEl("ul");C.style.cssText="list-style-type: none; padding-left: 0; margin: 0;";let F=(L,b,w,T)=>{if(L.length===0)return;let E=C.createEl("li");E.style.cssText=`${w} font-weight: bold; margin-bottom: 0.25em;`,E.setText(`${T} ${b} (${L.length}) :`);let N=C.createEl("ul");N.style.cssText="padding-left: 1.5em; margin-bottom: 0.5em; list-style-type: circle;";for(let M of L){let D=typeof M=="object"?`${M.name}${M.detail?" \u2014 "+M.detail:""}`:M;N.createEl("li",{text:D})}};F(this.syncedNotes,"Envoy\xE9e(s) / Mise(s) \xE0 jour \xE0 distance","color: var(--text-success);","\u2191"),F(this.pulledNotes,"T\xE9l\xE9charg\xE9e(s) / Mise(s) \xE0 jour localement","color: var(--text-success);","\u2193"),F(this.createdNotes,"Cr\xE9\xE9e(s) localement","color: var(--text-success);","+"),F(this.unpublishedNotes,"D\xE9-publi\xE9e(s) localement (pass\xE9e en brouillon)","color: var(--text-warning);","-"),F(this.skippedNotes,"D\xE9j\xE0 \xE0 jour (identiques)","color: var(--text-muted);","\u25CB"),F(this.failedNotes,"\xC9chec(s) de synchronisation","color: var(--text-error);","\u2717"),i?.failedImages?.length>0&&F(i.failedImages,"Image(s) non t\xE9l\xE9vers\xE9e(s)","color: var(--text-error);","\u2717"),this.actionBtn.disabled=!1,this.actionBtn.setText("Fermer"),this.actionBtn.removeClass("mod-warning"),this.actionBtn.addClass("mod-cta")}onClose(){this.finished||(this.cancelled=!0),this.contentEl.empty()}},ve=class extends R.Modal{constructor(e,t,a){super(e),this.garden=t,this.items=a,this.cancelled=!1,this.inProgress=!1}onOpen(){let{contentEl:e,titleEl:t}=this;e.addClass("stnd-modal"),t.setText("Standard \u2014 Nettoyage des notes d\xE9publi\xE9es"),e.createEl("p",{text:`Les ${this.items.length} note(s) suivante(s) sont encore en ligne sur votre jardin ou poss\xE8dent des m\xE9tadonn\xE9es de publication obsol\xE8tes, mais ne sont plus publiques localement :`,cls:"stnd-modal-detail"});let a=e.createDiv();a.style.cssText="max-height: 220px; overflow-y: auto; border: 1px solid var(--background-modifier-border); border-radius: 6px; padding: 8px 12px; margin: 12px 0; background: var(--background-primary-alt);";let n=a.createEl("ul");n.style.cssText="list-style-type: none; padding: 0; margin: 0;";for(let s of this.items){let i=n.createEl("li");i.style.cssText="display: flex; justify-content: space-between; align-items: center; padding: 4px 0; border-bottom: 1px solid var(--background-modifier-border-focus); font-size: var(--font-ui-smaller);";let l=i.createEl("span",{text:s.title});l.style.cssText="font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 68%;";let d="Brouillon local";s.reason==="orphan"?d="Orpheline en ligne":s.reason==="stale_local"&&(d="Lien local r\xE9siduel");let c=i.createEl("span",{text:d});c.style.cssText="color: var(--text-warning); font-size: 11px; padding: 2px 6px; border-radius: 4px; background: var(--background-modifier-hover); flex-shrink: 0;"}this.progressContainer=e.createDiv(),this.progressContainer.style.display="none",this.progressContainer.style.margin="12px 0";let o=this.progressContainer.createDiv();o.style.cssText="height: 8px; border-radius: 4px; background: var(--background-modifier-border); overflow: hidden;",this.barEl=o.createDiv(),this.barEl.style.cssText="height: 100%; width: 0%; background: var(--interactive-accent); transition: width .15s ease;",this.progressText=this.progressContainer.createEl("div",{cls:"stnd-modal-detail",text:"Traitement..."}),this.progressText.style.cssText="margin-top: 6px; font-size: var(--font-ui-smaller);";let r=e.createEl("div",{cls:"stnd-modal-btns"});this.cancelBtn=r.createEl("button",{text:"Annuler",cls:"stnd-modal-btn-cancel"}),this.cancelBtn.addEventListener("click",()=>{this.close()}),this.confirmBtn=r.createEl("button",{text:`Supprimer du jardin (${this.items.length})`,cls:"mod-warning"}),this.confirmBtn.addEventListener("click",async()=>{await this.runPrune()})}async runPrune(){this.inProgress=!0,this.confirmBtn.disabled=!0,this.cancelBtn.disabled=!0,this.confirmBtn.setText("Nettoyage en cours..."),this.progressContainer.style.display="block";let e=0,t=0,a=this.items.length;for(let n=0;n<a&&!this.cancelled;n++){let o=this.items[n],r=Math.round((n+1)/a*100);this.barEl.style.width=r+"%",this.progressText.setText(`${n+1} / ${a} : ${o.title}`);try{await this.garden.unpublishNote(o.file,o.slug)?e++:t++}catch{t++}}this.progressText.setText(`Termin\xE9 : ${e} note(s) nettoy\xE9e(s)${t>0?`, ${t} en \xE9chec`:""}.`),this.confirmBtn.style.display="none",this.cancelBtn.disabled=!1,this.cancelBtn.setText("Fermer"),this.cancelBtn.removeClass("stnd-modal-btn-cancel"),this.cancelBtn.addClass("mod-cta"),new R.Notice(`Standard : ${e} note(s) d\xE9publi\xE9e(s) du jardin.`)}onClose(){this.cancelled=!0,this.contentEl.empty()}};async function V(p,e={},t=5){let a=0,n=2e3;for(;;){a++;try{let o=await R.requestUrl({url:p,method:e.method||"GET",headers:e.headers||{},body:e.body,throw:!1}),r=()=>{try{return JSON.parse(o.text||"{}")}catch{return null}};if(o.status===429){if(a>=t)return{...o,ok:!1,json:async()=>r()};let s=o.headers&&(o.headers["retry-after"]||o.headers["Retry-After"]),i=0;if(s){let l=parseInt(s,10);i=isNaN(l)?n:l*1e3}else i=n+Math.random()*1e3,n*=2;console.warn(`Standard: Rate limited (429). Retrying in ${i}ms (attempt ${a}/${t})...`),await new Promise(l=>setTimeout(l,i));continue}return{...o,ok:o.status>=200&&o.status<300,json:async()=>r()}}catch(o){if(a>=t)throw o;let r=n+Math.random()*1e3;n*=2,console.warn(`Standard: Network error. Retrying in ${r}ms (attempt ${a}/${t})...`,o),await new Promise(s=>setTimeout(s,r))}}}async function W(p,e=null){if(p<=0)return;let t=Date.now();for(;Date.now()-t<p&&!(e&&e.cancelled);){let a=p-(Date.now()-t);await new Promise(n=>setTimeout(n,Math.min(50,a)))}}var ke=class{constructor(e,t){this.app=e,this.plugin=t,this.syncIntervalTimer=null,this.attachmentCache=new Map,this.lastAttachmentUploadTime=0,this.lastPublishTime=0}isPathExcluded(e){let t=this.plugin.settings.excludedFolders;if(!t)return!1;let a=Array.isArray(t)?t:String(t).split(",").map(o=>o.trim().replace(/^\/+|\/+$/g,"")).filter(Boolean);if(a.length===0)return!1;let n=e.replace(/^\/+/,"");return a.some(o=>n.startsWith(o+"/")||n===o)}getPublishableFiles(){return this.app.vault.getMarkdownFiles().filter(t=>!this.isPathExcluded(t.path))}async load(){this.plugin.settings.autoSync&&this.plugin.settings.apiKey&&setTimeout(()=>{this.pollRemoteChanges()},5e3),this.setupAutoSyncInterval()}unload(){this.syncIntervalTimer&&(clearInterval(this.syncIntervalTimer),this.syncIntervalTimer=null)}setupAutoSyncInterval(){if(this.syncIntervalTimer&&(clearInterval(this.syncIntervalTimer),this.syncIntervalTimer=null),!!this.plugin.settings.autoSync&&this.plugin.settings.apiKey){let t=window.setInterval(()=>{this.pollRemoteChanges()},3e5);this.syncIntervalTimer=t,this.plugin.registerInterval(t)}}async pollRemoteChanges(){if(this.plugin.settings.apiKey)try{let e=this.app.workspace.getActiveFile(),{PublishStatusFeature:t}=X(),a=this.plugin.features.find(n=>n instanceof t);a&&e&&await a.triggerStatusCheck(e)}catch{}}checkApiKeyAndShowModal(){return this.plugin.settings.apiKey?!0:(new we(this.app,`No API key configured.
To publish or edit a post, you must link your account.`,"Connect",()=>this.startConnect()).open(),!1)}async verifyApiKey(e,t){if(!this.plugin.settings.apiKey){this.plugin.settings.apiUsername="",await this.plugin.saveSettings(),this.plugin.updateRibbonIconsVisibility(),t&&t();return}try{let a=await R.requestUrl({url:`${this.plugin.settings.apiUrl}/me`,headers:{"x-api-key":this.plugin.settings.apiKey},throw:!1});if(a.status>=200&&a.status<300){let n=a.json;this.plugin.settings.apiUsername=n.username||"",await this.plugin.saveSettings(),this.plugin.updateRibbonIconsVisibility(),e&&e.setText(`\u2713 @${n.username}`)}else this.plugin.settings.apiUsername="",await this.plugin.saveSettings(),this.plugin.updateRibbonIconsVisibility(),e&&e.setText("\u2717 Invalid key")}catch{e&&e.setText("\u2717 Could not connect")}t&&t()}startConnect(){let e=(this.plugin.settings.apiUrl||"https://standard.garden/api").replace(/\/api\/?$/,""),t=typeof crypto<"u"&&crypto.randomUUID?crypto.randomUUID():String(Math.random()).slice(2);this._connectState=t;let a=`${e}/connect/obsidian?state=${encodeURIComponent(t)}`;this._openExternal(a),new R.Notice("Standard : connexion ouverte dans le navigateur\u2026")}_openExternal(e){if(R.Platform.isDesktop)try{require("electron").shell.openExternal(e);return}catch{}window.open(e,"_blank")}async handleConnectCallback(e){let t=e&&e.key,a=e&&e.username||"",n=e&&e.state||"";if(!t){new R.Notice("Garden: connection cancelled (missing key).");return}if(this._connectState&&n!==this._connectState){new R.Notice("Garden: connection skipped (invalid token).");return}this._connectState=null,this.plugin.settings.apiKey=t,this.plugin.settings.apiUsername=a,await this.plugin.saveSettings(),await this.verifyApiKey(null,()=>{this.plugin.settingTab&&this.plugin.settingTab.display()});let o=this.plugin.settings.apiUsername||a;new R.Notice(o?`Garden: connected as @${o} \u2713`:"Garden: connected \u2713")}async syncAllPublished(){if(!this.checkApiKeyAndShowModal())return;let e=this.getPublishableFiles(),t=(this.plugin.settings.keyPrefix||"")+this.plugin.settings.publishKey;try{let a=await V(`${this.plugin.settings.apiUrl}/publish?includeContent=true`,{method:"GET",headers:{"x-api-key":this.plugin.settings.apiKey}}),n=[];a.ok?n=(await a.json()).notes||[]:console.warn("Standard : Impossible de r\xE9cup\xE9rer la liste distante pour la r\xE9conciliation.");let o=new Map(n.map(b=>[b.slug,b])),r=new Map(n.filter(b=>b.nano_id).map(b=>[b.nano_id,b])),s=new Map;for(let b of n)if(b.title){let w=I(b.title);w&&!s.has(w)&&s.set(w,b)}let i=new Z(this.app,e),l=(b,w)=>{if(w["garden-short"]){let N=String(w["garden-short"]).match(/stnd\.gd\/([a-zA-Z0-9_-]+)/);if(N&&r.has(N[1]))return r.get(N[1])}let T=w.permalink??w.slug;if(T!=null){let N=String(T).replace(/^\/+|\/+$/g,""),M=N===""?"~root":N;if(o.has(M))return o.get(M);if(s.has(M))return s.get(M)}let E=I(b.basename);if(o.has(E))return o.get(E);if(s.has(E))return s.get(E);if(w.title){let N=I(w.title);if(o.has(N))return o.get(N);if(s.has(N))return s.get(N)}return null},d=new Set,c=[];for(let b of e){let w=this.app.metadataCache.getFileCache(b)?.frontmatter||{},T=w.permalink??w.slug,E=I(b.basename),N=T!=null?String(T).replace(/^\/+|\/+$/g,""):E,M=N===""?"~root":N,D=G(w),A=w["garden-url"]!=null,P=l(b,w);P&&d.add(P),D?c.push({type:"local_published",file:b,remoteNote:P,slug:M,name:b.basename}):P?c.push({type:"local_draft_remote_exists",file:b,remoteNote:P,slug:M,name:b.basename}):A&&this.plugin.settings.syncDirection==="2way"&&c.push({type:"local_published",file:b,remoteNote:P,slug:M,name:b.basename})}if(this.plugin.settings.syncDirection==="2way")for(let b of n)!d.has(b)&&!i.findMatchForRemote(b)&&c.push({type:"remote_only",file:null,remoteNote:b,slug:b.slug,name:b.title||b.slug});if(c.length===0){new R.Notice("Standard : Aucune note \xE0 synchroniser.");return}let h={toPublish:c.filter(b=>b.type==="local_published").length,toUnpublish:c.filter(b=>b.type==="local_draft_remote_exists").length,toCreate:c.filter(b=>b.type==="remote_only").length},f=new be(this.app,c.length,h);f.open();let u=0,m=0,g=0,y=0,v=0,k=0,x=0,C={imagesChecked:0,imagesReused:0,imagesUploaded:0,imagesFailed:0,failedImages:[]},F=new Set;for(let b of c){if(f.cancelled||(f.update({index:x,current:b.name,synced:u,pulled:m,created:g,unpublished:y,skipped:v,failed:k}),f.cancelled))break;try{if(b.type==="local_published"){let{file:w,remoteNote:T}=b,N=(this.app.metadataCache.getFileCache(w)?.frontmatter||{})["garden-url"]!=null;if(T){let M=await this.app.vault.read(w),D=await this.uploadContentImages(M,w,!0,C,f,F),A=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(D)),U=Array.from(new Uint8Array(A)).map(B=>B.toString(16).padStart(2,"0")).join("");if(T.hash===U)v++,f.recordResult("skipped",w.basename);else if(this.plugin.settings.syncDirection==="1way"){let B=Date.now()-this.lastPublishTime;if(B<1100&&await W(1100-B,f),f.cancelled)break;let ce=await this.publishNote(w,!0,D);this.lastPublishTime=Date.now(),ce?(u++,f.recordResult("synced",w.basename)):(k++,f.recordResult("failed",w.basename,this.lastError||"Erreur de publication"))}else{let B=w.stat?.mtime||0;if((new Date(T.updated_at).getTime()||0)>B+5e3)await this.app.vault.modify(w,T.content),m++,f.recordResult("pulled",w.basename);else{let ne=Date.now()-this.lastPublishTime;if(ne<1100&&await W(1100-ne,f),f.cancelled)break;let Pt=await this.publishNote(w,!0,D);this.lastPublishTime=Date.now(),Pt?(u++,f.recordResult("synced",w.basename)):(k++,f.recordResult("failed",w.basename,this.lastError||"Erreur de publication"))}}}else if(N)if(this.plugin.settings.syncDirection==="1way"){let M=await this.app.vault.read(w),D=await this.uploadContentImages(M,w,!0,C,f,F),A=Date.now()-this.lastPublishTime;if(A<1100&&await W(1100-A,f),f.cancelled)break;let P=await this.publishNote(w,!0,D);this.lastPublishTime=Date.now(),P?(u++,f.recordResult("synced",w.basename)):(k++,f.recordResult("failed",w.basename,this.lastError||"Erreur de publication"))}else await this.app.fileManager.processFrontMatter(w,M=>{M[t]=!1,delete M["garden-url"],delete M["garden-short"]}),y++,f.recordResult("unpublished",w.basename);else{let M=await this.app.vault.read(w),D=await this.uploadContentImages(M,w,!0,C,f,F),A=Date.now()-this.lastPublishTime;if(A<1100&&await W(1100-A,f),f.cancelled)break;let P=await this.publishNote(w,!0,D);this.lastPublishTime=Date.now(),P?(u++,f.recordResult("synced",w.basename)):(k++,f.recordResult("failed",w.basename,this.lastError||"Erreur de publication"))}}else if(b.type==="local_draft_remote_exists"){let{file:w,remoteNote:T}=b,E=w.stat?.mtime||0,N=new Date(T.updated_at).getTime()||0;if(this.plugin.settings.syncDirection==="2way"&&N>E+5e3)await this.app.vault.modify(w,T.content),await this.app.fileManager.processFrontMatter(w,D=>{G(D)||(D.status="public"),D["garden-url"]=this.getLiveUrl(w),T.nano_id&&(D["garden-short"]=`https://stnd.gd/${T.nano_id}`),D.created!=null&&T.created_at&&(D.created=T.created_at),D.modified!=null&&T.updated_at&&(D.modified=T.updated_at)}),m++,f.recordResult("pulled",w.basename);else{let D=b.remoteNote?.slug||b.slug,A=Date.now()-this.lastPublishTime;if(A<1100&&await W(1100-A,f),f.cancelled)break;let P=await this.unpublishNote(w,D);this.lastPublishTime=Date.now(),P?(y++,f.recordResult("unpublished",w.basename)):(k++,f.recordResult("failed",w.basename,this.lastError||"Erreur de d\xE9publication"))}}else if(b.type==="remote_only"){let{remoteNote:w,slug:T}=b,E=(w.title||T).replace(/[\\\/:\*\?"<>\|]/g,"-").trim();E||(E="Sans titre");let N=`${E}.md`,M=1;for(;this.app.vault.getAbstractFileByPath(N);)N=`${E} (${M}).md`,M++;let D=await this.app.vault.create(N,w.content);await this.app.fileManager.processFrontMatter(D,A=>{G(A)||(A.status="public"),A["garden-url"]=this.getLiveUrl(D),A.permalink=w.slug,w.nano_id&&(A["garden-short"]=`https://stnd.gd/${w.nano_id}`),A.created!=null&&w.created_at&&(A.created=w.created_at),A.modified!=null&&w.updated_at&&(A.modified=w.updated_at)}),g++,f.recordResult("created",D.basename)}}catch(w){console.error(`Standard : Erreur lors de la synchronisation de ${b.name}:`,w),k++,f.recordResult("failed",b.name)}x++,f.update({index:x,current:b.name,synced:u,pulled:m,created:g,unpublished:y,skipped:v,failed:k})}let L=n.length+u-y;f.done({synced:u,pulled:m,created:g,unpublished:y,skipped:v,failed:k,notesOnline:L,stats:C}),new R.Notice(`Garden : Synchronisation ${f.cancelled?"annul\xE9e":"termin\xE9e"}. ${u+m+g+y} action(s), ${v} identique(s), ${k} en \xE9chec.`)}catch(a){console.error("Standard : Erreur globale lors de la synchronisation en lot :",a),new R.Notice("Standard : Erreur lors de la synchronisation.")}}async downloadNewOnlineNotes(){if(!this.checkApiKeyAndShowModal())return;let e=this.getPublishableFiles(),t=(this.plugin.settings.keyPrefix||"")+this.plugin.settings.publishKey;try{let a=await V(`${this.plugin.settings.apiUrl}/publish?includeContent=true`,{method:"GET",headers:{"x-api-key":this.plugin.settings.apiKey}}),n=[];if(a.ok)n=(await a.json()).notes||[];else{new R.Notice("Garden: Failed to fetch online notes list.");return}let o=new Z(this.app,e),r=[],s=[];for(let l of n){let d=o.findMatchForRemote(l);d?s.push({remoteNote:l,file:d}):r.push(l)}for(let{remoteNote:l,file:d}of s)await this.app.fileManager.processFrontMatter(d,c=>{G(c)||(c.status="public"),c["garden-url"]||(c["garden-url"]=this.getLiveUrl(d)),!c.permalink&&l.slug&&l.slug!==I(d.basename)&&(c.permalink=l.slug),l.nano_id&&!c["garden-short"]&&(c["garden-short"]=`https://stnd.gd/${l.nano_id}`),c.created!=null&&l.created_at&&(c.created=l.created_at),c.modified!=null&&l.updated_at&&(c.modified=l.updated_at)});if(r.length===0){new R.Notice(s.length>0?`Garden : Toutes les ${s.length} notes sont d\xE9j\xE0 dans le coffre (m\xE9tadonn\xE9es synchronis\xE9es).`:"Garden : Aucune nouvelle note en ligne \xE0 t\xE9l\xE9charger.");return}let i=0;for(let l of r){let d=(l.title||l.slug).replace(/[\\\/:\*\?"<>\|]/g,"-").trim();d||(d="Untitled");let c=`${d}.md`,h=1;for(;this.app.vault.getAbstractFileByPath(c);)c=`${d} (${h}).md`,h++;let f=await this.app.vault.create(c,l.content);await this.app.fileManager.processFrontMatter(f,u=>{G(u)||(u.status="public"),u["garden-url"]=this.getLiveUrl(f),u.permalink=l.slug,l.nano_id&&(u["garden-short"]=`https://stnd.gd/${l.nano_id}`),u.created!=null&&l.created_at&&(u.created=l.created_at),u.modified!=null&&l.updated_at&&(u.modified=l.updated_at)}),i++}new R.Notice(`Garden: Downloaded ${i} new note(s) successfully!`)}catch(a){console.error("Garden: Error during downloading online notes:",a),new R.Notice("Garden: Failed to download online notes.")}}async publishCurrentNote(){if(!this.checkApiKeyAndShowModal())return;let e=this.app.workspace.getActiveFile();if(!e){new R.Notice("Standard: No active note to publish.");return}if(this.isPathExcluded(e.path)){new R.Notice(`Standard : Cette note se trouve dans un dossier exclu de la publication (${e.path}).`);return}let t=await this.publishWithCheck(e);t===!0?(new R.Notice(`Standard: "${e.basename}" published.`),this.plugin.settings.openAfterPublish&&this.viewLiveVersion(e)):t===!1&&new R.Notice(`Standard: Failed to publish "${e.basename}".`)}async unpublishCurrentNote(){if(!this.checkApiKeyAndShowModal())return;let e=this.app.workspace.getActiveFile();if(!e){new R.Notice("Standard: No active note to unpublish.");return}let t=await this.deleteOnlineVersion(e);t===!0?new R.Notice(`Standard: "${e.basename}" removed from the garden.`):t===!1&&new R.Notice(`Standard: Failed to unpublish "${e.basename}".`)}async cleanUnpublishedNotes(){if(this.checkApiKeyAndShowModal()){new R.Notice("Standard : Recherche des notes \xE0 d\xE9publier...");try{let e=await V(`${this.plugin.settings.apiUrl}/publish`,{method:"GET",headers:{"x-api-key":this.plugin.settings.apiKey}}),t=[];if(e.ok)t=(await e.json()).notes||[];else{new R.Notice("Standard : Impossible de r\xE9cup\xE9rer les notes distantes.");return}let a=this.getPublishableFiles(),n=new Z(this.app,a),o=(this.plugin.settings.keyPrefix||"")+this.plugin.settings.publishKey,r=[],s=new Set;for(let i of t){let l=n.findMatchForRemote(i),d=i.slug||"~root";if(!l)r.push({slug:d,title:i.title||i.slug||"Sans titre",file:null,reason:"orphan"});else{s.add(l);let c=this.app.metadataCache.getFileCache(l)?.frontmatter||{};G(c)||r.push({slug:d,title:l.basename,file:l,reason:"draft"})}}for(let i of a){if(s.has(i))continue;let l=this.app.metadataCache.getFileCache(i)?.frontmatter||{};if(!G(l)&&(l["garden-url"]||l["garden-short"])){let d=l.permalink??l.slug,c=I(i.basename),h=d!=null?String(d).replace(/^\/+|\/+$/g,""):c,f=h===""?"~root":h;r.push({slug:f,title:i.basename,file:i,reason:"stale_local"})}}if(r.length===0){new R.Notice("Standard : Aucune note d\xE9publi\xE9e \xE0 nettoyer. Le jardin est parfaitement synchronis\xE9 !");return}new ve(this.app,this,r).open()}catch(e){console.error("Standard : Erreur lors du nettoyage :",e),new R.Notice("Standard : Erreur lors de la recherche des notes.")}}}getGardenDomain(){try{let e=this.app.vault.getMarkdownFiles();for(let t of e){let n=this.app.metadataCache.getFileCache(t)?.frontmatter||{},o=n["garden-domain"]??n.garden_domain??n.domain;if(n.permalink==="/"&&o)return String(o).trim().replace(/^https?:\/\//,"").replace(/\/$/,"")}}catch(e){console.error("Standard: Error scanning for garden-domain:",e)}return null}getLiveUrl(e){let t=this.app.metadataCache.getFileCache(e)?.frontmatter||{};if(t["garden-url"])return String(t["garden-url"]).trim();let a=t.permalink??t.slug,n=I(e.basename),o=a!=null?String(a).replace(/^\/+|\/+$/g,""):n,r=this.getGardenDomain();if(r)return o===""?`https://${r}`:`https://${r}/${o}`;{let s="https://standard.garden";return o===""?`${s}/@${this.plugin.settings.apiUsername}`:`${s}/@${this.plugin.settings.apiUsername}/${o}`}}viewLiveVersion(e){let t=e||this.app.workspace.getActiveFile();if(!t){new R.Notice("Standard: No active note.");return}let a=this.getLiveUrl(t);this.plugin.settings.openInObsidianWeb?this.app.workspace.getLeaf("split","vertical").setViewState({type:"webviewer",state:{url:a}}):this._openExternal(a)}stripFrontmatter(e){return e?e.replace(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/,""):""}extractMarkdownSubpath(e,t){if(!e||!t)return e;let a=t.trim();if(!a)return e;if(a.startsWith("^")){let l=a.slice(1),d=e.split(`
`),c=[],h=!1;for(let f=0;f<d.length;f++)if(d[f].includes(`^${l}`)){h=!0;let u=f;for(;u>0&&d[u-1].trim()!==""&&!d[u-1].startsWith("#");)u--;let m=f;for(;m<d.length-1&&d[m+1].trim()!==""&&!d[m+1].startsWith("#");)m++;for(let g=u;g<=m;g++)c.push(d[g].replace(new RegExp(`\\s*\\^${l}\\s*$`),""));break}return h?c.join(`
`).trim():e}let n=a.replace(/^#+/,"").trim().toLowerCase(),o=e.split(`
`),r=!1,s=0,i=[];for(let l=0;l<o.length;l++){let d=o[l],c=d.match(/^(#{1,6})\s+(.+)$/);if(c){let h=c[1].length,f=c[2].replace(/[#*`_\[\]]/g,"").trim().toLowerCase();if(r){if(h<=s)break;i.push(d)}else(f===n||f.startsWith(n))&&(r=!0,s=h,i.push(d))}else r&&i.push(d)}return r&&i.length>0?i.join(`
`).trim():e}async resolveNoteTransclusions(e,t,a=0,n=new Set){if(!e||a>5)return e;let o=t?t.path:"",r=new Set(n);o&&r.add(o);let s=/!\[\[([^\]]+)\]\]/g,i=[...e.matchAll(s)];if(i.length===0)return e;let l=e;for(let d of i){let c=d[0],h=d[1],[f]=h.split("|"),u=f.trim();if(ye(u)||re(u)||lt(u))continue;let m=u.indexOf("#"),g=m!==-1?u.slice(0,m).trim():u,y=m!==-1?u.slice(m+1).trim():null,v=null;if(g?v=this.app.metadataCache.getFirstLinkpathDest(g,o):t&&(v=t),!(!v||v.extension!=="md")){if(r.has(v.path)&&a>0&&!y){console.warn(`Standard: Circular transclusion detected for ${v.path}`),l=l.replace(c,"");continue}try{let k=await this.app.vault.read(v),x=this.stripFrontmatter(k);y&&(x=this.extractMarkdownSubpath(x,y));let C=new Set(r);C.add(v.path);let F=await this.resolveNoteTransclusions(x,v,a+1,C);l=l.replace(c,F.trim())}catch(k){console.warn(`Standard: Error resolving transclusion ${c}:`,k)}}}return l}async uploadContentImages(e,t,a=!1,n=null,o=null,r=null){if(!this.plugin.settings.apiKey)return e;let s=await this.resolveNoteTransclusions(e,t),i=new Map,l=async u=>{if(o&&o.cancelled)return null;if(i.has(u.path))return i.get(u.path);try{let m=u.stat?.mtime||0,g=u.stat?.size||0,y=u.name.split(".").pop()?.toLowerCase()||"bin",v="",k="",x=this.attachmentCache?.get(u.path);if(x&&x.mtime===m&&x.size===g&&x.contentHash)v=x.contentHash,k=x.cdnUrl;else{let U=await this.app.vault.readBinary(u),B=await crypto.subtle.digest("SHA-256",U);v=Array.from(new Uint8Array(B)).map(ne=>ne.toString(16).padStart(2,"0")).join("")}if(r&&r.has(v)&&k)return i.set(u.path,k),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesReused=(n.imagesReused||0)+1),k;let C=`${this.plugin.settings.apiUrl}/publish/attachment?hash=${v}&ext=${y}`,F=await V(C,{method:"GET",headers:{"x-api-key":this.plugin.settings.apiKey}});if(F.status>=200&&F.status<300){let U=await F.json();if(U?.exists&&U.url)return k=new URL(U.url,this.plugin.settings.apiUrl).href,this.attachmentCache&&this.attachmentCache.set(u.path,{mtime:m,size:g,contentHash:v,cdnUrl:k}),r&&r.add(v),i.set(u.path,k),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesReused=(n.imagesReused||0)+1),k}if(a){let U=Date.now()-this.lastAttachmentUploadTime;if(U<2e3&&await W(2e3-U,o),o&&o.cancelled)return null}let L=await this.app.vault.readBinary(u),b="----ObsidianBoundary"+Math.random().toString(36).substring(2),w=[`--${b}`,`Content-Disposition: form-data; name="file"; filename="${u.name}"`,`Content-Type: ${en(u.name)}`,"",""].join(`\r
`),T=`\r
--${b}--`,E=new Uint8Array(L),N=new TextEncoder().encode(w),M=new TextEncoder().encode(T),D=new Uint8Array(N.length+E.byteLength+M.length);D.set(N,0),D.set(E,N.length),D.set(M,N.length+E.byteLength);let A=await V(`${this.plugin.settings.apiUrl}/publish/attachment`,{method:"POST",headers:{"x-api-key":this.plugin.settings.apiKey,"Content-Type":`multipart/form-data; boundary=${b}`},body:D.buffer});if(this.lastAttachmentUploadTime=Date.now(),A.status<200||A.status>=300){let U=`HTTP ${A.status}`;try{let B=await A.json();B?.error&&(U=B.error)}catch{}return console.warn(`Standard: Failed to upload ${u.name}: ${U}`,A.status),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesFailed=(n.imagesFailed||0)+1,n.failedImages&&n.failedImages.push({name:u.name,detail:U})),null}let P=await A.json();return P?.url?(k=new URL(P.url,this.plugin.settings.apiUrl).href,this.attachmentCache&&this.attachmentCache.set(u.path,{mtime:m,size:g,contentHash:v,cdnUrl:k}),r&&r.add(v),i.set(u.path,k),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesUploaded=(n.imagesUploaded||0)+1),k):(console.warn(`Standard: ${u.name} accept\xE9 par le serveur sans URL en retour`,P),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesFailed=(n.imagesFailed||0)+1,n.failedImages&&n.failedImages.push({name:u.name,detail:"R\xE9ponse serveur sans URL"})),null)}catch(m){return console.warn(`Standard: Error uploading ${u.name}`,m),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesFailed=(n.imagesFailed||0)+1,n.failedImages&&n.failedImages.push({name:u.name,detail:m.message||String(m)})),null}},d=s,c=/!\[\[([^\]]+)\]\]/g;for(let u of[...d.matchAll(c)]){let m=u[1],g=m.split("|")[0].trim();if(!ye(g))continue;let y=this.app.metadataCache.getFirstLinkpathDest(g,t.path);if(!y)continue;let v=await l(y);if(!v)continue;let k=m.includes("|")?m.split("|")[1]:y.basename;if(re(g))d=d.replaceAll(u[0],`![${k}](${v})`);else if(lt(g))d=d.replaceAll(u[0],`[${k}](${v} "pdf-embed")`);else{let x=`${v}?download=${encodeURIComponent(y.name)}`;d=d.replaceAll(u[0],`[${k}](${x})`)}}let h=/(^|[^!])\[\[([^\]]+)\]\]/g;for(let u of[...d.matchAll(h)]){let m=u[2],g=m.split("|")[0].trim();if(!ye(g)||re(g))continue;let y=this.app.metadataCache.getFirstLinkpathDest(g,t.path);if(!y)continue;let v=await l(y);if(!v)continue;let k=m.includes("|")?m.split("|")[1]:y.name,x=`${v}?download=${encodeURIComponent(y.name)}`;d=d.replaceAll(u[0],`${u[1]}[${k}](${x})`)}let f=/!\[([^\]]*)\]\(([^)]+)\)/g;for(let u of[...d.matchAll(f)]){let m=u[2];if(/^https?:\/\//.test(m)||!re(m))continue;let g=decodeURIComponent(m),y=this.app.vault.getFileByPath(g)||this.app.metadataCache.getFirstLinkpathDest(g,t.path);if(!y)continue;let v=await l(y);v&&(d=d.replaceAll(u[0],`![${u[1]}](${v})`))}return d}async publishNote(e,t=!1,a=null){try{let n=a!==null?a:await this.uploadContentImages(await this.app.vault.read(e),e,t),o=this.app.metadataCache.getFileCache(e)?.frontmatter||{},r=o.permalink??o.slug,s=I(e.basename),i=r!=null?String(r).replace(/^\/+|\/+$/g,""):s,l=i===""?"~root":i,d=o.created||(e.stat?.ctime?new Date(e.stat.ctime).toISOString():new Date().toISOString()),c=o.modified||(e.stat?.mtime?new Date(e.stat.mtime).toISOString():new Date().toISOString()),h=await V(`${this.plugin.settings.apiUrl}/publish/${encodeURIComponent(l)}`,{method:"PUT",headers:{"Content-Type":"application/json","x-api-key":this.plugin.settings.apiKey},body:JSON.stringify({title:e.basename,content:n,slug:l,created_at:d,updated_at:c})});if(h.status<200||h.status>=300){let m=`HTTP ${h.status}`;try{let g=JSON.parse(h.text||"{}");g.error&&(m+=`: ${g.error}`)}catch{h.text&&(m+=`: ${h.text.slice(0,80)}`)}return this.lastError=m,console.error(`Standard: Publish failed for ${e.basename}:`,h.status,h.text),!1}let f=await h.json().catch(()=>null),u=this.getLiveUrl(e);return await this.app.fileManager.processFrontMatter(e,m=>{delete m.published,delete m.url_public,m["garden-url"]=u,i!==""&&(m.permalink=i),f&&f.nano_id&&(m["garden-short"]=`https://stnd.gd/${f.nano_id}`),m.modified!=null&&(m.modified=new Date().toISOString())}),!0}catch(n){return this.lastError=n.message||String(n),console.error(`Standard: Publish error for ${e.basename}:`,n),!1}}async unpublishNote(e,t=null){try{let a=t;if(!a&&e){let o=this.app.metadataCache.getFileCache(e)?.frontmatter||{},r=o.permalink??o.slug,s=I(e.basename),i=r!=null?String(r).replace(/^\/+|\/+$/g,""):s;a=i===""?"~root":i}if(!a)return!1;let n=await V(`${this.plugin.settings.apiUrl}/publish/${encodeURIComponent(a)}`,{method:"DELETE",headers:{"x-api-key":this.plugin.settings.apiKey}});if(n.status!==404&&(n.status<200||n.status>=300)){let o=`HTTP ${n.status}`;try{let r=JSON.parse(n.text||"{}");r.error&&(o+=`: ${r.error}`)}catch{n.text&&(o+=`: ${n.text.slice(0,80)}`)}return this.lastError=o,console.error(`Standard: Unpublish failed for ${e?e.basename:a}:`,n.status),!1}return e&&await this.app.fileManager.processFrontMatter(e,o=>{(o.status==="public"||o.status==="published")&&(o.status="draft"),o.publish!==void 0&&(o.publish=!1),delete o.published,delete o.url_public,delete o["garden-url"],delete o["garden-short"]}),!0}catch(a){return console.error(`Standard: Unpublish error for ${e?e.basename:t}:`,a),!1}}async generateTokens(e,t,a){let n=this.plugin.settings.apiKey;if(!n)throw new Error("Connect your Standard Garden account in settings first.");let o=Object.fromEntries(Object.entries(a).filter(([i])=>$t.has(i))),r=await R.requestUrl({url:`${this.plugin.settings.apiUrl}/ai/theme`,method:"POST",headers:{"Content-Type":"application/json","x-api-key":n},body:JSON.stringify({instruction:e,noteContent:t,currentTokens:o}),throw:!1});if(r.status<200||r.status>=300){let i=r.text;throw new Error(`Standard API ${r.status}: ${i}`)}let s=r.json;if(!s.tokens)throw new Error("No tokens returned from AI service");return s.tokens}async publishWithCheck(e){if(!this.checkApiKeyAndShowModal())return null;let a=this.app.metadataCache.getFileCache(e)?.frontmatter||{},n=async()=>(await this.app.fileManager.processFrontMatter(e,s=>{G(s)||(s.status="public")}),await this.publishNote(e)),o=[];return(a.status==="draft"||a.publish===!1||a.publish==="false")&&o.push("\u2022 status: draft \u2014 it asked to stay a draft"),String(a.visibility||"").toLowerCase()==="private"&&o.push("\u2022 visibility: private \u2014 visitors won't see it"),o.length>0?new Promise(r=>{new we(this.app,`\u{1F331} Plant "${e.basename}" in the garden anyway?

${o.join(`
`)}`,"Plant it",async()=>r(await n()),()=>r(null)).open()}):await n()}async checkNoteStatus(e){if(!this.plugin.settings.apiKey||this.isPathExcluded(e.path))return{status:"unpublished"};try{let t=this.app.metadataCache.getFileCache(e)?.frontmatter||{},a=t.permalink??t.slug,n=I(e.basename),o=a!=null?String(a).replace(/^\/+|\/+$/g,""):n,r=o===""?"~root":o,s=await V(`${this.plugin.settings.apiUrl}/publish/${encodeURIComponent(r)}`,{method:"GET",headers:{"x-api-key":this.plugin.settings.apiKey}});if(s.status===404)return{status:"unpublished"};if(s.status<200||s.status>=300)throw new Error(`HTTP error ${s.status}`);let i=await s.json(),l=i.content||"",d=i.hash,c=i.updated_at?new Date(i.updated_at).getTime():0,h=await this.app.vault.read(e),f=C=>{if(!C)return"";let F=C.replace(/\r\n/g,`
`);return F=F.replace(/!\[\[([^\]]+)\]\]/g,(L,b)=>{let w=b.split("|"),T=w[0].trim(),E=w[1]?w[1].trim():"";return E?`[IMAGE:${E.toLowerCase()}]`:`[IMAGE:${(T.substring(0,T.lastIndexOf("."))||T).toLowerCase()}]`}),F=F.replace(/!\[([^\]]*)\]\(([^)]+)\)/g,(L,b,w)=>{let T=b.trim();if(T)return`[IMAGE:${T.toLowerCase()}]`;let E=w.split("/").pop()||"";return`[IMAGE:${(E.substring(0,E.lastIndexOf("."))||E).toLowerCase()}]`}),F.trim()},u=f(h),m=f(l),g=new TextEncoder().encode(u),y=await crypto.subtle.digest("SHA-256",g),k=Array.from(new Uint8Array(y)).map(C=>C.toString(16).padStart(2,"0")).join("");if(d===k)return{status:"synced"};let x=e.stat.mtime;return c>x+5e3?{status:"outdated",remoteContent:l}:{status:"changed"}}catch(t){return console.error("Standard : Erreur lors de la v\xE9rification du statut de la note :",t),{status:"error"}}}async deleteOnlineVersion(e){return this.checkApiKeyAndShowModal()?new Promise(t=>{new we(this.app,`Remove "${e.basename}" from Standard Garden?

The online version will be permanently deleted.`,"Delete online version",async()=>{let a=await this.unpublishNote(e);t(a)},()=>t(null)).open()}):null}async getNoteStats(e){if(!this.plugin.settings.apiKey||this.isPathExcluded(e.path))return null;try{let t=this.app.metadataCache.getFileCache(e)?.frontmatter||{},a=t.permalink??t.slug,n=I(e.basename),o=a!=null?String(a).replace(/^\/+|\/+$/g,""):n,r=o===""?"~root":o,s=await V(`${this.plugin.settings.apiUrl}/publish/${encodeURIComponent(r)}`,{method:"GET",headers:{"x-api-key":this.plugin.settings.apiKey}});if(s.status===200){let i=await s.json();return{views:i.views||0,citations:Array.isArray(i.citations)?i.citations:[],created_at:i.created_at,updated_at:i.updated_at,online:!0}}return null}catch(t){return console.error("[Standard] Error fetching note stats:",t),null}}async askGardenAI(){this.checkApiKeyAndShowModal()&&new tn(this.app,this.plugin).open()}};dt.exports={GardenFeature:ke}});var pt=O((aa,ft)=>{"use strict";var{EditorSuggest:nn,setTooltip:an}=require("obsidian"),xe=[{id:"feed",name:"::feed",category:"Flux",syntax:"::feed #tag",description:"Flux de cartes visuelles avec image cover, date relative et extrait markdown.",insertText:"::feed #",cursorOffset:8},{id:"list",name:"::list",category:"Flux",syntax:"::list #tag",description:"Liste compacte \xE0 puces des notes li\xE9es au tag sp\xE9cifi\xE9.",insertText:"::list #",cursorOffset:8},{id:"callout",name:"::callout",category:"Bloc",syntax:`::callout note
...
