...
`);return D=D.replace(/!\[\[([^\]]+)\]\]/g,(E,b)=>{let w=b.split("|"),T=w[0].trim(),M=w[1]?w[1].trim():"";return M?`[IMAGE:${M.toLowerCase()}]`:`[IMAGE:${(T.substring(0,T.lastIndexOf("."))||T).toLowerCase()}]`}),D=D.replace(/!\[([^\]]*)\]\(([^)]+)\)/g,(E,b,w)=>{let T=b.trim();if(T)return`[IMAGE:${T.toLowerCase()}]`;let M=w.split("/").pop()||"";return`[IMAGE:${(M.substring(0,M.lastIndexOf("."))||M).toLowerCase()}]`}),D.trim()},h=c(u),m=c(l),g=new TextEncoder().encode(h),y=await crypto.subtle.digest("SHA-256",g),k=Array.from(new Uint8Array(y)).map(L=>L.toString(16).padStart(2,"0")).join("");if(d===k)return{status:"synced"};let x=e.stat.mtime;return f>x+5e3?{status:"outdated",remoteContent:l}:{status:"changed"}}catch(t){return console.error("Standard : Erreur lors de la v\xE9rification du statut de la note :",t),{status:"error"}}}async deleteOnlineVersion(e){return this.checkApiKeyAndShowModal()?new Promise(t=>{new ke(this.app,`Remove "${e.basename}" from Standard Garden?
