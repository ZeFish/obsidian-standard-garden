...
`).trim():e}async resolveNoteTransclusions(e,t,a=0,n=new Set){if(!e||a>5)return e;let o=t?t.path:"",r=new Set(n);o&&r.add(o);let s=/!\[\[([^\]]+)\]\]/g,i=[...e.matchAll(s)];if(i.length===0)return e;let l=e;for(let d of i){let f=d[0],u=d[1],[c]=u.split("|"),h=c.trim();if(ve(h)||se(h)||pt(h))continue;let m=h.indexOf("#"),g=m!==-1?h.slice(0,m).trim():h,y=m!==-1?h.slice(m+1).trim():null,b=null;if(g?b=this.app.metadataCache.getFirstLinkpathDest(g,o):t&&(b=t),!(!b||b.extension!=="md")){if(r.has(b.path)&&a>0&&!y){console.warn(`Standard: Circular transclusion detected for ${b.path}`),l=l.replace(f,"");continue}try{let k=await this.app.vault.read(b),x=this.stripFrontmatter(k);y&&(x=this.extractMarkdownSubpath(x,y));let L=new Set(r);L.add(b.path);let D=await this.resolveNoteTransclusions(x,b,a+1,L);l=l.replace(f,D.trim())}catch(k){console.warn(`Standard: Error resolving transclusion ${f}:`,k)}}}return l}async uploadContentImages(e,t,a=!1,n=null,o=null,r=null){if(!this.plugin.settings.apiKey)return e;let s=await this.resolveNoteTransclusions(e,t),i=new Map,l=async h=>{if(o&&o.cancelled)return null;if(i.has(h.path))return i.get(h.path);try{let m=h.stat?.mtime||0,g=h.stat?.size||0,y=h.name.split(".").pop()?.toLowerCase()||"bin",b="",k="",x=this.attachmentCache?.get(h.path);if(x&&x.mtime===m&&x.size===g&&x.contentHash)b=x.contentHash,k=x.cdnUrl;else{let U=await this.app.vault.readBinary(h),B=await crypto.subtle.digest("SHA-256",U);b=Array.from(new Uint8Array(B)).map(ae=>ae.toString(16).padStart(2,"0")).join("")}if(r&&r.has(b)&&k)return i.set(h.path,k),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesReused=(n.imagesReused||0)+1),k;let L=`${this.plugin.settings.apiUrl}/publish/attachment?hash=${b}&ext=${y}`,D=await z(L,{method:"GET",headers:{"x-api-key":this.plugin.settings.apiKey}});if(D.status>=200&&D.status<300){let U=await D.json();if(U?.exists&&U.url)return k=new URL(U.url,this.plugin.settings.apiUrl).href,this.attachmentCache&&this.attachmentCache.set(h.path,{mtime:m,size:g,contentHash:b,cdnUrl:k}),r&&r.add(b),i.set(h.path,k),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesReused=(n.imagesReused||0)+1),k}if(a){let U=Date.now()-this.lastAttachmentUploadTime;if(U<2e3&&await G(2e3-U,o),o&&o.cancelled)return null}let E=await this.app.vault.readBinary(h),v="----ObsidianBoundary"+Math.random().toString(36).substring(2),w=[`--${v}`,`Content-Disposition: form-data; name="file"; filename="${h.name}"`,`Content-Type: ${rn(h.name)}`,"",""].join(`\r
`),T=`\r
--${v}--`,M=new Uint8Array(E),F=new TextEncoder().encode(w),A=new TextEncoder().encode(T),C=new Uint8Array(F.length+M.byteLength+A.length);C.set(F,0),C.set(M,F.length),C.set(A,F.length+M.byteLength);let R=await z(`${this.plugin.settings.apiUrl}/publish/attachment`,{method:"POST",headers:{"x-api-key":this.plugin.settings.apiKey,"Content-Type":`multipart/form-data; boundary=${v}`},body:C.buffer});if(this.lastAttachmentUploadTime=Date.now(),R.status<200||R.status>=300){let U=`HTTP ${R.status}`;try{let B=await R.json();B?.error&&(U=B.error)}catch{}return console.warn(`Standard: Failed to upload ${h.name}: ${U}`,R.status),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesFailed=(n.imagesFailed||0)+1,n.failedImages&&n.failedImages.push({name:h.name,detail:U})),null}let P=await R.json();return P?.url?(k=new URL(P.url,this.plugin.settings.apiUrl).href,this.attachmentCache&&this.attachmentCache.set(h.path,{mtime:m,size:g,contentHash:b,cdnUrl:k}),r&&r.add(b),i.set(h.path,k),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesUploaded=(n.imagesUploaded||0)+1),k):(console.warn(`Standard: ${h.name} accept\xE9 par le serveur sans URL en retour`,P),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesFailed=(n.imagesFailed||0)+1,n.failedImages&&n.failedImages.push({name:h.name,detail:"R\xE9ponse serveur sans URL"})),null)}catch(m){return console.warn(`Standard: Error uploading ${h.name}`,m),n&&(n.imagesChecked=(n.imagesChecked||0)+1,n.imagesFailed=(n.imagesFailed||0)+1,n.failedImages&&n.failedImages.push({name:h.name,detail:m.message||String(m)})),null}},d=s,f=/!\[\[([^\]]+)\]\]/g;for(let h of[...d.matchAll(f)]){let m=h[1],g=m.split("|")[0].trim();if(!ve(g))continue;let y=this.app.metadataCache.getFirstLinkpathDest(g,t.path);if(!y)continue;let b=await l(y);if(!b)continue;let k=m.includes("|")?m.split("|")[1]:y.basename;if(se(g))d=d.replaceAll(h[0],`![${k}](${b})`);else if(pt(g))d=d.replaceAll(h[0],`[${k}](${b} "pdf-embed")`);else{let x=`${b}?download=${encodeURIComponent(y.name)}`;d=d.replaceAll(h[0],`[${k}](${x})`)}}let u=/(^|[^!])\[\[([^\]]+)\]\]/g;for(let h of[...d.matchAll(u)]){let m=h[2],g=m.split("|")[0].trim();if(!ve(g)||se(g))continue;let y=this.app.metadataCache.getFirstLinkpathDest(g,t.path);if(!y)continue;let b=await l(y);if(!b)continue;let k=m.includes("|")?m.split("|")[1]:y.name,x=`${b}?download=${encodeURIComponent(y.name)}`;d=d.replaceAll(h[0],`${h[1]}[${k}](${x})`)}let c=/!\[([^\]]*)\]\(([^)]+)\)/g;for(let h of[...d.matchAll(c)]){let m=h[2];if(/^https?:\/\//.test(m)||!se(m))continue;let g=decodeURIComponent(m),y=this.app.vault.getFileByPath(g)||this.app.metadataCache.getFirstLinkpathDest(g,t.path);if(!y)continue;let b=await l(y);b&&(d=d.replaceAll(h[0],`![${h[1]}](${b})`))}return d}async publishNote(e,t=!1,a=null){try{let n=a!==null?a:await this.uploadContentImages(await this.app.vault.read(e),e,t),o=this.app.metadataCache.getFileCache(e)?.frontmatter||{},r=o.permalink??o.slug,s=I(e.basename),i=r!=null?String(r).replace(/^\/+|\/+$/g,""):s,l=i===""?"~root":i,d=o.created||(e.stat?.ctime?new Date(e.stat.ctime).toISOString():new Date().toISOString()),f=o.modified||(e.stat?.mtime?new Date(e.stat.mtime).toISOString():new Date().toISOString()),u=await z(`${this.plugin.settings.apiUrl}/publish/${encodeURIComponent(l)}`,{method:"PUT",headers:{"Content-Type":"application/json","x-api-key":this.plugin.settings.apiKey},body:JSON.stringify({title:e.basename,content:n,slug:l,created_at:d,updated_at:f})});if(u.status<200||u.status>=300){let m=`HTTP ${u.status}`;try{let g=JSON.parse(u.text||"{}");g.error&&(m+=`: ${g.error}`)}catch{u.text&&(m+=`: ${u.text.slice(0,80)}`)}return this.lastError=m,console.error(`Standard: Publish failed for ${e.basename}:`,u.status,u.text),!1}let c=await u.json().catch(()=>null),h=this.getLiveUrl(e);return await this.app.fileManager.processFrontMatter(e,m=>{delete m.published,delete m.url_public,m["garden-url"]=h,i!==""&&(m.permalink=i),c&&c.nano_id&&(m["garden-short"]=`https://stnd.gd/${c.nano_id}`),m.modified!=null&&(m.modified=new Date().toISOString())}),!0}catch(n){return this.lastError=n.message||String(n),console.error(`Standard: Publish error for ${e.basename}:`,n),!1}}async unpublishNote(e,t=null){try{let a=t;if(!a&&e){let o=this.app.metadataCache.getFileCache(e)?.frontmatter||{},r=o.permalink??o.slug,s=I(e.basename),i=r!=null?String(r).replace(/^\/+|\/+$/g,""):s;a=i===""?"~root":i}if(!a)return!1;let n=await z(`${this.plugin.settings.apiUrl}/publish/${encodeURIComponent(a)}`,{method:"DELETE",headers:{"x-api-key":this.plugin.settings.apiKey}});if(n.status!==404&&(n.status<200||n.status>=300)){let o=`HTTP ${n.status}`;try{let r=JSON.parse(n.text||"{}");r.error&&(o+=`: ${r.error}`)}catch{n.text&&(o+=`: ${n.text.slice(0,80)}`)}return this.lastError=o,console.error(`Standard: Unpublish failed for ${e?e.basename:a}:`,n.status),!1}return e&&await this.app.fileManager.processFrontMatter(e,o=>{(o.status==="public"||o.status==="published")&&(o.status="draft"),o.publish!==void 0&&(o.publish=!1),delete o.published,delete o.url_public,delete o["garden-url"],delete o["garden-short"]}),!0}catch(a){return console.error(`Standard: Unpublish error for ${e?e.basename:t}:`,a),!1}}async generateTokens(e,t,a){let n=this.plugin.settings.apiKey;if(!n)throw new Error("Connect your Standard Garden account in settings first.");let o=Object.fromEntries(Object.entries(a).filter(([i])=>on.has(i))),r=await N.requestUrl({url:`${this.plugin.settings.apiUrl}/ai/theme`,method:"POST",headers:{"Content-Type":"application/json","x-api-key":n},body:JSON.stringify({instruction:e,noteContent:t,currentTokens:o}),throw:!1});if(r.status<200||r.status>=300){let i=r.text;throw new Error(`Standard API ${r.status}: ${i}`)}let s=r.json;if(!s.tokens)throw new Error("No tokens returned from AI service");return s.tokens}async publishWithCheck(e){if(!this.checkApiKeyAndShowModal())return null;let a=this.app.metadataCache.getFileCache(e)?.frontmatter||{},n=async()=>(await this.app.fileManager.processFrontMatter(e,s=>{W(s)||(s.status="public")}),await this.publishNote(e)),o=[];return(a.status==="draft"||a.publish===!1||a.publish==="false")&&o.push("\u2022 status: draft \u2014 it asked to stay a draft"),String(a.visibility||"").toLowerCase()==="private"&&o.push("\u2022 visibility: private \u2014 visitors won't see it"),o.length>0?new Promise(r=>{new ke(this.app,`\u{1F331} Plant "${e.basename}" in the garden anyway?

${o.join(`
`)}`,"Plant it",async()=>r(await n()),()=>r(null)).open()}):await n()}async checkNoteStatus(e){if(!this.plugin.settings.apiKey||this.isPathExcluded(e.path))return{status:"unpublished"};try{let t=this.app.metadataCache.getFileCache(e)?.frontmatter||{},a=t.permalink??t.slug,n=I(e.basename),o=a!=null?String(a).replace(/^\/+|\/+$/g,""):n,r=o===""?"~root":o,s=await z(`${this.plugin.settings.apiUrl}/publish/${encodeURIComponent(r)}`,{method:"GET",headers:{"x-api-key":this.plugin.settings.apiKey}});if(s.status===404)return{status:"unpublished"};if(s.status<200||s.status>=300)throw new Error(`HTTP error ${s.status}`);let i=await s.json(),l=i.content||"",d=i.hash,f=i.updated_at?new Date(i.updated_at).getTime():0,u=await this.app.vault.read(e),c=L=>{if(!L)return"";let D=L.replace(/\r\n/g,`
`);return D=D.replace(/!\[\[([^\]]+)\]\]/g,(E,v)=>{let w=v.split("|"),T=w[0].trim(),M=w[1]?w[1].trim():"";return M?`[IMAGE:${M.toLowerCase()}]`:`[IMAGE:${(T.substring(0,T.lastIndexOf("."))||T).toLowerCase()}]`}),D=D.replace(/!\[([^\]]*)\]\(([^)]+)\)/g,(E,v,w)=>{let T=v.trim();if(T)return`[IMAGE:${T.toLowerCase()}]`;let M=w.split("/").pop()||"";return`[IMAGE:${(M.substring(0,M.lastIndexOf("."))||M).toLowerCase()}]`}),D.trim()},h=c(u),m=c(l),g=new TextEncoder().encode(h),y=await crypto.subtle.digest("SHA-256",g),k=Array.from(new Uint8Array(y)).map(L=>L.toString(16).padStart(2,"0")).join("");if(d===k)return{status:"synced"};let x=e.stat.mtime;return f>x+5e3?{status:"outdated",remoteContent:l}:{status:"changed"}}catch(t){return console.error("Standard : Erreur lors de la v\xE9rification du statut de la note :",t),{status:"error"}}}async deleteOnlineVersion(e){return this.checkApiKeyAndShowModal()?new Promise(t=>{new ke(this.app,`Remove "${e.basename}" from Standard Garden?
